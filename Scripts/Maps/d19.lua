...
evt.map[1] = function()  -- function events.LoadMap()
	...
	if evt.Cmp{"QBits", Value = 229} then         -- You have Pissed off the Necros
		if not evt.Cmp{"Counter9", Value = 1344} then
			goto _5
		end
		evt.SetMonGroupBit{NPCGroup = 10, Bit = const.MonsterBits.Hostile, On = false}         -- ""
		...
		evt.Subtract{"QBits", Value = 229}         -- You have Pissed off the Necros
	end
::_17::
...
::_5::
	...
	evt.SetMonGroupBit{NPCGroup = 10, Bit = const.MonsterBits.Invisible, On = false}         -- ""
	evt.SetMonGroupBit{NPCGroup = 10, Bit = const.MonsterBits.Hostile, On = true}         -- ""
...
evt.map[6] = function()  -- function events.LeaveMap()
	if evt.Cmp{"QBits", Value = 19} then         -- Allied with Necromancers Guild. Steal Nightshade Brazier done.
		...
		evt.MoveNPC{NPC = 56, HouseId = 751}         -- "Sandro" -> "Council Chamber Door"
		evt.MoveNPC{NPC = 63, HouseId = 213}         -- "Thant" -> "Sandro/Thant's Throne Room"
	end
	if not evt.Cmp{"QBits", Value = 229} then         -- You have Pissed off the Necros
		if evt.Cmp{"MapVar9", Value = 2} then
			evt.Add{"QBits", Value = 229}         -- You have Pissed off the Necros
			evt.Set{"Counter9", Value = 0}
		else
...
evt.map[15] = function()
	...
	if evt.Cmp{"Players", Value = 34} then
...
evt.map[18] = function()
	if evt.Cmp{"MapVar19", Value = 15} then
		...
		if evt.Cmp{"MapVar19", Value = 6} then
			evt.Subtract{"MapVar19", Value = 1}
		end
	end
end
...
evt.map[19] = function()
	if not evt.Cmp{"MapVar19", Value = 15} then
		if evt.Cmp{"MapVar19", Value = 5} then
			evt.SetDoorState{Id = 8, State = 1}
			evt.SetDoorState{Id = 9, State = 0}
			evt.SetDoorState{Id = 10, State = 1}
			evt.Add{"MapVar19", Value = 1}
			return
		end
	end
	evt.StatusText{Str = 10}         -- "The Door will not move"
end

evt.hint[20] = evt.str[4]  -- "Lever"
evt.map[20] = function()
	if not evt.Cmp{"MapVar19", Value = 15} then
		if not evt.Cmp{"MapVar19", Value = 6} then
			if not evt.Cmp{"MapVar19", Value = 5} then
				if not evt.Cmp{"MapVar19", Value = 4} then
					if not evt.Cmp{"MapVar19", Value = 3} then
						if not evt.Cmp{"MapVar19", Value = 2} then
							if evt.Cmp{"MapVar19", Value = 1} then
								evt.SetDoorState{Id = 11, State = 0}
								evt.Subtract{"MapVar19", Value = 1}
							else
	...
	evt.StatusText{Str = 7}         -- "The lever will not move"
end
...
evt.map[21] = function()
	if not evt.Cmp{"MapVar19", Value = 15} then
		if not evt.Cmp{"MapVar19", Value = 6} then
			if not evt.Cmp{"MapVar19", Value = 5} then
				if not evt.Cmp{"MapVar19", Value = 4} then
					if evt.Cmp{"MapVar19", Value = 3} then
						evt.SetDoorState{Id = 12, State = 0}
						evt.Subtract{"MapVar19", Value = 1}
						return
					end
					if evt.Cmp{"MapVar19", Value = 2} then
						evt.SetDoorState{Id = 12, State = 1}
						evt.Add{"MapVar19", Value = 1}
						return
					end
					evt.Cmp{"MapVar19", Value = 1}
				end
			end
		end
	end
...
evt.map[23] = function()
	if not evt.Cmp{"MapVar19", Value = 15} then
		if not evt.Cmp{"MapVar19", Value = 6} then
			...
			if evt.Cmp{"MapVar19", Value = 4} then
...
evt.map[25] = function()
	if not evt.Cmp{"MapVar19", Value = 6} then
		evt.StatusText{Str = 8}         -- "The Beam will not move"
	elseif evt.Cmp{"MapVar19", Value = 15} then
		evt.SetDoorState{Id = 16, State = 0}
		evt.Set{"MapVar19", Value = 6}
	else
		evt.SetDoorState{Id = 16, State = 1}
		evt.Set{"MapVar19", Value = 15}
	end
end
...
evt.map[61] = function()
	...
	if evt.Cmp{"QBits", Value = 19} then         -- Allied with Necromancers Guild. Steal Nightshade Brazier done.
		if evt.Cmp{"MapVar9", Value = 1} then
			evt.Set{"QBits", Value = 229}         -- You have Pissed off the Necros
			evt.Set{"Counter9", Value = 0}
			goto _12
		end
		evt.Set{"MapVar9", Value = 1}
	end
	evt.SpeakNPC{NPC = 34}         -- "Guard"
	evt.FaceAnimation{Player = "Current", Animation = 18}
	do return end
::_17::
...
evt.house[201] = 213  -- "Sandro/Thant's Throne Room"
evt.map[201] = function()
	...
	if not evt.Cmp{"QBits", Value = 19} then         -- Allied with Necromancers Guild. Steal Nightshade Brazier done.
		evt.ForPlayer("All")
		if evt.Cmp{"Inventory", Value = 604} then         -- "Nightshade Brazier"
			evt.Subtract{"Inventory", Value = 604}         -- "Nightshade Brazier"
			...
			evt.Add{"QBits", Value = 19}         -- Allied with Necromancers Guild. Steal Nightshade Brazier done.
			...
			evt.ForPlayer("All")
			evt.Add{"Awards", Value = 10}         -- "Formed an alliance with the Necromancers' Guild."
			evt.Add{"Experience", Value = 12500}
			evt.ForPlayer("Current")
			evt.Add{"Gold", Value = 8000}
			evt.ShowMovie{DoubleSize = 1, Name = "\"nightshade\""}
			evt.Add{"History10", Value = 0}
			evt.Subtract{"QBits", Value = 203}         -- Nightshade Brazier - I lost it
		end
	end
	evt.EnterHouse{Id = 213}         -- "Sandro/Thant's Throne Room"
	do return end
::_27::
	evt.FaceAnimation{Player = "Current", Animation = 18}
end

evt.hint[203] = evt.str[100]  -- ""
evt.map[203] = function()
	evt.Set{"MapVar29", Value = 5}
	if evt.Cmp{"QBits", Value = 89} then         -- Dyson Leland talks to you about the Necromancers. For global event 97-100.
		goto _4
	end
	if evt.Cmp{"QBits", Value = 90} then         -- Dyson Leland talks to you about the Temple of the Sun. For Global event 97-100.
		goto _4
	end
::_15::
	evt.EnterHouse{Id = 754}         -- "Dyson Leland's Room"
	do return end
::_4::
	...
	if evt.Cmp{"QBits", Value = 28} then         -- "Bring the Nightshade Brazier to the Necromancers' Guild leader, Sandro. The Brazier is in the Temple of the Sun."
		if evt.Cmp{"MapVar29", Value = 5} then
...
evt.map[451] = function()
	if not evt.Cmp{"MapVar9", Value = 2} then
		evt.Set{"MapVar9", Value = 0}
	end
end
