...
evt.map[2] = function()  -- function events.LoadMap()
	if evt.Cmp{"QBits", Value = 6} then         -- Pirate Leader in Dagger Wound Pirate Outpost killed (quest given at Q Bit 5). Ends pirate/lizardman war on Dagger Wound. Shuts off pirate timer.
		...
		evt.SetMonGroupBit{NPCGroup = 14, Bit = const.MonsterBits.Invisible, On = true}         -- ""
		evt.SetMonGroupBit{NPCGroup = 10, Bit = const.MonsterBits.Invisible, On = true}         -- ""
		evt.SetMonGroupBit{NPCGroup = 11, Bit = const.MonsterBits.Invisible, On = true}         -- ""
		evt.SetNPCGroupNews{NPCGroup = 12, NPCNews = 13}         -- "" : "With the Pirate Outpost defeated our homes are once again safe!  Thank you!"
		evt.SetNPCGroupNews{NPCGroup = 13, NPCNews = 13}         -- "" : "With the Pirate Outpost defeated our homes are once again safe!  Thank you!"
		evt.SetNPCGroupNews{NPCGroup = 1, NPCNews = 2}         -- "" : "Our thanks for defeating the pirates!  Now if you could only do something about the mountain of fire!"
	else
		evt.SetNPCGroupNews{NPCGroup = 12, NPCNews = 12}         -- "" : "Help us!  If we do not push the pirates off the island all will be lost!"
		evt.SetNPCGroupNews{NPCGroup = 13, NPCNews = 12}         -- "" : "Help us!  If we do not push the pirates off the island all will be lost!"
		if not evt.Cmp{"QBits", Value = 36} then         -- "Sink the Regnan Fleet. Return to the Ravenshore council chamber."
...
evt.map[3] = function()  -- function events.LoadMap()
	if not evt.Cmp{"QBits", Value = 226} then         -- game Init stuff in area one
		evt.Add{"QBits", Value = 226}         -- game Init stuff in area one
		evt.Add{"QBits", Value = 185}         -- Blood Drop Town Portal
		evt.Add{"QBits", Value = 401}         -- Roster Character In Party 2
		evt.Add{"QBits", Value = 407}         -- Roster Character In Party 8
	end
end
...
evt.hint[11] = evt.str[100]  -- ""
evt.map[11] = function()
	evt.EnterHouse{Id = 761}         -- "Hiss' Hut"
end

evt.hint[12] = evt.str[100]  -- ""
evt.hint[13] = evt.str[100]  -- ""
evt.map[13] = function()
	evt.EnterHouse{Id = 776}         -- "Rohtnax's House"
end

evt.hint[14] = evt.str[100]  -- ""
evt.hint[15] = evt.str[100]  -- ""
evt.map[15] = function()
	evt.EnterHouse{Id = 784}         -- "Tisk's Hut"
end

evt.hint[16] = evt.str[100]  -- ""
evt.hint[17] = evt.str[100]  -- ""
evt.map[17] = function()
	evt.EnterHouse{Id = 792}         -- "Thadin's House"
end

evt.hint[18] = evt.str[100]  -- ""
evt.hint[19] = evt.str[100]  -- ""
evt.map[19] = function()
	evt.EnterHouse{Id = 795}         -- "House of Ich"
end

evt.hint[20] = evt.str[100]  -- ""
evt.hint[21] = evt.str[100]  -- ""
evt.map[21] = function()
	evt.EnterHouse{Id = 798}         -- "Languid's Hut"
end

evt.hint[22] = evt.str[100]  -- ""
evt.hint[23] = evt.str[100]  -- ""
evt.map[23] = function()
	evt.EnterHouse{Id = 804}         -- "House of Thistle"
end

evt.hint[24] = evt.str[100]  -- ""
evt.hint[25] = evt.str[100]  -- ""
evt.map[25] = function()
	evt.EnterHouse{Id = 813}         -- "Zevah's Hut"
end

evt.hint[26] = evt.str[100]  -- ""
evt.hint[27] = evt.str[100]  -- ""
evt.map[27] = function()
	evt.EnterHouse{Id = 823}         -- "Isthric's House"
end

evt.hint[28] = evt.str[100]  -- ""
evt.hint[29] = evt.str[100]  -- ""
evt.map[29] = function()
	evt.EnterHouse{Id = 833}         -- "Bone's House"
end

evt.hint[30] = evt.str[100]  -- ""
evt.hint[31] = evt.str[100]  -- ""
evt.map[31] = function()
	evt.EnterHouse{Id = 843}         -- "Lasatin's Hut"
end

evt.hint[32] = evt.str[100]  -- ""
evt.hint[33] = evt.str[100]  -- ""
evt.map[33] = function()
	evt.EnterHouse{Id = 852}         -- "Menasaur's House"
end

evt.hint[34] = evt.str[100]  -- ""
evt.hint[35] = evt.str[100]  -- ""
evt.map[35] = function()
	evt.EnterHouse{Id = 860}         -- "Husk's Hut"
end

evt.hint[36] = evt.str[100]  -- ""
evt.hint[37] = evt.str[100]  -- ""
evt.map[37] = function()
	evt.EnterHouse{Id = 866}         -- "Talimere's Hut"
end

evt.hint[38] = evt.str[100]  -- ""
evt.hint[39] = evt.str[100]  -- ""
evt.map[39] = function()
	evt.EnterHouse{Id = 870}         -- "House of Reshie"
end

evt.hint[40] = evt.str[100]  -- ""
evt.hint[41] = evt.str[100]  -- ""
evt.map[41] = function()
	evt.EnterHouse{Id = 873}         -- "House"
end

evt.hint[42] = evt.str[100]  -- ""
evt.hint[43] = evt.str[100]  -- ""
evt.map[43] = function()
	evt.EnterHouse{Id = 876}         -- "Long-Tail's Hut"
end

evt.hint[44] = evt.str[100]  -- ""
evt.hint[45] = evt.str[100]  -- ""
evt.map[45] = function()
	evt.EnterHouse{Id = 879}         -- "Aislen's House"
end

evt.hint[46] = evt.str[100]  -- ""
evt.hint[47] = evt.str[100]  -- ""
evt.map[47] = function()
	evt.EnterHouse{Id = 882}         -- "House of Grivic"
end

evt.hint[48] = evt.str[100]  -- ""
evt.hint[49] = evt.str[100]  -- ""
evt.map[49] = function()
	evt.EnterHouse{Id = 885}         -- "Ush's Hut"
end

evt.hint[50] = evt.str[100]  -- ""
evt.hint[81] = evt.str[3]  -- "Chest"
evt.map[81] = function()
	if evt.Cmp{"Players", Value = 2} then
		evt.OpenChest{Id = 3}
	else
		evt.OpenChest{Id = 0}
	end
...
evt.map[101] = function()
	if evt.Cmp{"IntellectBonus", Value = 15} then
		evt.StatusText{Str = 9}         -- "Refreshing"
	else
		evt.Add{"IntellectBonus", Value = 15}
		evt.StatusText{Str = 60}         -- "Intellect +15 (Temporary)"
		evt.Add{"AutonotesBits", Value = 206}         -- "Well in the village of Blood Drop on Dagger Wound Island gives a temporary Intellect bonus of 15."
	end
end

evt.hint[102] = evt.str[6]  -- "Drink from the well"
evt.map[102] = function()
	if evt.Cmp{"BaseLuck", Value = 16} then
		evt.StatusText{Str = 9}         -- "Refreshing"
	else
		evt.Add{"BaseLuck", Value = 2}
		evt.StatusText{Str = 61}         -- "Luck +2 (Permanent)"
		evt.Add{"AutonotesBits", Value = 207}         -- "Well in the village of Blood Drop on Dagger Wound Island gives a  permanent Luck bonus up to a Luck of 16."
	end
end

evt.hint[103] = evt.str[6]  -- "Drink from the well"
evt.map[103] = function()
	if not evt.Cmp{"MapVar29", Value = 2} then
		if not evt.Cmp{"Gold", Value = 99} then
			if not evt.Cmp{"BankGold", Value = 99} then
				if evt.Cmp{"BaseLuck", Value = 14} then
					evt.Add{"Gold", Value = 1000}
					evt.Add{"MapVar29", Value = 1}
					evt.Add{"AutonotesBits", Value = 208}         -- "Well in the village of Blood Drop on Dagger Wound Island gives 1000 gold if Luck is greater than 14 and total gold on party and in the bank is less than 100."
					return
				end
			end
		end
...
evt.map[104] = function()
	if evt.Cmp{"HasFullHP", Value = 0} then
		...
		evt.Add{"HP", Value = 25}
		evt.StatusText{Str = 62}         -- "Your Wounds begin to Heal"
		evt.Add{"AutonotesBits", Value = 209}         -- "Fountain in the village of Blood Drop on Dagger Wound Island restores Hit Points."
	end
end

evt.hint[150] = evt.str[33]  -- "Obelisk"
evt.map[150] = function()
	if not evt.Cmp{"QBits", Value = 186} then         -- Obelisk Area 1
		evt.StatusText{Str = 12}         -- "summerday"
		evt.Add{"AutonotesBits", Value = 16}         -- "Obelisk message #9: summerday"
		evt.Add{"QBits", Value = 186}         -- Obelisk Area 1
	end
end

evt.HouseDoor(171, 1)  -- "True Mettle"
evt.house[172] = 1  -- "True Mettle"
evt.HouseDoor(173, 36)  -- "The Tannery"
evt.house[174] = 36  -- "The Tannery"
evt.HouseDoor(175, 76)  -- "Fearsome Fetishes"
evt.house[176] = 76  -- "Fearsome Fetishes"
evt.HouseDoor(177, 110)  -- "Herbal Elixirs"
evt.house[178] = 110  -- "Herbal Elixirs"
evt.HouseDoor(179, 182)  -- "Cures and Curses"
evt.house[180] = 182  -- "Cures and Curses"
evt.HouseDoor(183, 479)  -- "The Windling"
evt.house[184] = 479  -- "The Windling"
evt.HouseDoor(185, 303)  -- "Mystic Medicine"
evt.house[186] = 303  -- "Mystic Medicine"
evt.hint[187] = evt.str[100]  -- ""
evt.map[187] = function()
	evt.EnterHouse{Id = 1564}         -- "Rites of Passage"
end

evt.hint[188] = evt.str[100]  -- ""
evt.HouseDoor(191, 228)  -- "The Grog and Grub"
evt.house[192] = 228  -- "The Grog and Grub"
evt.HouseDoor(193, 281)  -- "The Some Place Safe"
evt.house[194] = 281  -- "The Some Place Safe"
evt.HouseDoor(197, 212)  -- "Clan Leader's Hall"
evt.house[198] = 212  -- "Clan Leader's Hall"
evt.hint[199] = evt.str[100]  -- ""
evt.map[199] = function()
	evt.EnterHouse{Id = 1607}         -- "The Adventurer's Inn"
end

evt.hint[200] = evt.str[100]  -- ""
...
evt.map[453] = function()
	if not evt.Cmp{"QBits", Value = 1} then         -- Activate Area 1 teleporters 3 and 4.
		evt.ForPlayer("All")
		if not evt.Cmp{"Inventory", Value = 617} then         -- "Power Stone"
			evt.StatusText{Str = 14}         -- "You need a power stone to operate this teleporter"
			return
		end
		if not evt.Cmp{"QBits", Value = 8} then         -- Fredrick Talimere visited by player with crystal in their possesion.
			return
		end
		evt.Subtract{"Inventory", Value = 617}         -- "Power Stone"
		evt.Add{"QBits", Value = 1}         -- Activate Area 1 teleporters 3 and 4.
	end
...
evt.map[466] = function()
	if not evt.Cmp{"QBits", Value = 2} then         -- Activate Area 1 teleporters 5 and 6.
		evt.ForPlayer("All")
		if not evt.Cmp{"Inventory", Value = 618} then         -- "Power Stone"
			evt.StatusText{Str = 14}         -- "You need a power stone to operate this teleporter"
			return
		end
		evt.Subtract{"Inventory", Value = 618}         -- "Power Stone"
		evt.Add{"QBits", Value = 2}         -- Activate Area 1 teleporters 5 and 6.
	end
...
evt.map[468] = function()  -- Timer(<function>, 20*const.Minute)
	local i
	if not evt.Cmp{"QBits", Value = 6} then         -- Pirate Leader in Dagger Wound Pirate Outpost killed (quest given at Q Bit 5). Ends pirate/lizardman war on Dagger Wound. Shuts off pirate timer.
		i = Game.Rand() % 5
		if i == 1 then
...
evt.map[471] = function()
	...
	if not evt.Cmp{"QBits", Value = 227} then         -- Turn on Temple Bypass tele in area one
		evt.Add{"QBits", Value = 227}         -- Turn on Temple Bypass tele in area one
	end
end

evt.hint[472] = evt.str[100]  -- ""
evt.map[472] = function()
	if evt.Cmp{"QBits", Value = 227} then         -- Turn on Temple Bypass tele in area one
...
evt.map[494] = function()
	local i
	if evt.Cmp{"QBits", Value = 270} then         -- Reagent spout area 1
		return
	end
	if not evt.Cmp{"PerceptionSkill", Value = 3} then
		return
	end
	i = Game.Rand() % 5
	if i == 1 then
	...
	evt.Add{"QBits", Value = 270}         -- Reagent spout area 1
end
...
evt.map[495] = function()
	...
	i = Game.Rand() % 4
	if i == 1 then
		evt.SummonObject{Item = 2138, X = -18832, Y = 5840, Z = 330, Speed = 1000, Count = 1, RandomAngle = true}         -- "Gold Ring" : "Diary Page"
	elseif i == 2 then
		evt.SummonObject{Item = 2139, X = -18832, Y = 5840, Z = 330, Speed = 1000, Count = 1, RandomAngle = true}         -- "Pearl Ring" : "Page from Agar's Journal"
	elseif i == 3 then
		evt.SummonObject{Item = 2140, X = -18832, Y = 5840, Z = 330, Speed = 1000, Count = 1, RandomAngle = true}         -- "Gemstone Ring" : "Remains of a Journal"
	else
		evt.SummonObject{Item = 2141, X = -18832, Y = 5840, Z = 330, Speed = 1000, Count = 1, RandomAngle = true}         -- "Amethyst Ring" : "Letter from the Dragon Riders"
	end
...
evt.map[497] = function()
	if not evt.Cmp{"QBits", Value = 268} then         -- Area 1 buoy
		if evt.Cmp{"BaseLuck", Value = 13} then
			evt.Add{"QBits", Value = 268}         -- Area 1 buoy
...
evt.map[498] = function()
	if not evt.Cmp{"QBits", Value = 269} then         -- Area 1 buoy
		if evt.Cmp{"BaseLuck", Value = 20} then
			evt.Add{"QBits", Value = 269}         -- Area 1 buoy
...
evt.map[500] = function()  -- Timer(<function>, const.Day, 10*const.Hour, false)
	if not evt.Cmp{"QBits", Value = 232} then         -- Set when you talk to S'ton
		evt.Add{"QBits", Value = 232}         -- Set when you talk to S'ton
		evt.SpeakNPC{NPC = 27}         -- "S'ton"
	end
end
