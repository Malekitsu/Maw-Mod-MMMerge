...
function events.BuildStatInformationBox(t)
	...
	if t.Stat==8 then
		...
		local medRegen = math.round(fullSP^0.25*s^1.75*(m+5)/50)+2
		--meditation buff
		if buffRework and vars.mawbuff[56] then
			local s, m, level=getBuffSkill(56)
			local level=level^0.6
			medRegen = medRegen + math.round((fullSP^0.25*level^1.75*((buffPower[56].Base[m])/150) +10)*(1+buffPower[56].Scaling[m]/100*s))
		end
...
function events.CalcDamageToMonster(t)
	...
	if t.Player and vars.legendaries and vars.legendaries[t.PlayerIndex] and table.find(vars.legendaries[t.PlayerIndex], 29) then
		...
		if table.find(aoespells, data.Spell) and math.random()>0.4 then
		for i=0, 10 do
			if i~=5 then
				if i==4 then
					local id=t.Monster:GetIndex()
