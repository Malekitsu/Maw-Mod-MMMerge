...
function events.GameInitialized2()
	...
	Game.SkillDesGM[const.Skills.Spear]=string.format("%s each attack reduces monster phys resistance by 2+0.05 per skill point, multiplied by your base weapon speed",Game.SkillDesGM[const.Skills.Spear])
...
function events.Tick()
	if Game.CurrentCharScreen==101 and Game.CurrentScreen==7 then
		...
		local regenEffect={[0]=0,3,4,6,6}
		local hpRegen = math.round(FHP^0.5*s^1.75*((regenEffect[m])/35))/10
		local hpRegen2 = math.round(FHP^0.5*(s+1)^1.75*((regenEffect[m])/35))/10
...
function events.Tick()
	if Game.CurrentCharScreen==101 and Game.CurrentScreen==7 then
		...
		local efficiency=math.round((1+s^1.5/125*4)*100)/100
		if s > 50 then 
