-- zzzOnlineVariableSender.lua — diff-only health sync, single Tick driver, QoL always-open

----------------------------------------------------------------
-- ONE-TIME GUARD
----------------------------------------------------------------
if __ONLINE_SENDER_LOADED__ then return end
__ONLINE_SENDER_LOADED__ = true
...
-- Toggle (kept from original): open shops/transports 24/7 while in multiplayer
...
local function getDistance(x, y, z)
  local dx, dy, dz = (Party.X - x), (Party.Y - y), (Party.Z - z)
  return math.sqrt(dx*dx + dy*dy + dz*dz)
end
_G.getDistance = _G.getDistance or getDistance

-- Small quiet window after joins/reloads to avoid bursty traffic
...
----------------------------------------------------------------
-- LIGHTWEIGHT SCHEDULER (MawTimer) — single Tick driver
----------------------------------------------------------------
...
----------------------------------------------------------------
-- HEALTH/MANA DIFF SYNC
----------------------------------------------------------------
local function packPartySnapshot()
  local snap = { High = Party.High, Map = Map.Name }
  snap.X, snap.Y, snap.Z = Party.X, Party.Y, Party.Z
...
do
  ...
  function events.Tick()
    ...
    if timePassed < 0 or timePassed > hour then timePassed = 0 end
    local irlSeconds = timePassed / 128
    if irlSeconds > 0 then MawTimer(irlSeconds) end
    lastTime = Game.Time
...
function OnlineLowestHealthPercentage()
  ...
  if Multiplayer and Multiplayer.in_game then
    for PartyId, party in pairs(vars.online.partyHealthMana.Parties) do
      if party.Map == Map.Name and getDistance(party.X or 0, party.Y or 0, party.Z or 0) < 4000 then
