...
----------------------------------------------------------------
-- LIGHTWEIGHT SCHEDULER (MawTimer)
----------------------------------------------------------------
local MawTimers = _G.MawTimers or {}
_G.MawTimers = MawTimers

function MawAddTimer(name, interval, fn)
  assert(type(fn) == "function", "MawAddTimer('"..tostring(name).."'): fn must be a function")
  MawTimers[name] = { interval = tonumber(interval) or 1, acc = 0, fn = fn, enabled = true }
end
function MawEnableTimer(name, enabled) local t=MawTimers[name]; if t then t.enabled=(enabled~=false) end end
...
do
  local hour = 15360
  local lastTime = nil
  local FALLBACK_DT = 1/60   -- ✅ dt de secours si le temps recule/stagne
  local MAX_DT      = 0.25   -- borne anti-burst

  function events.Tick()
    ensure_online_state()

    -- Driver de timers
    lastTime = lastTime or Game.Time
    local timePassed = Game.Time - lastTime
    local dt
...
-- TIMER REGISTRATION
----------------------------------------------------------------
MawAddTimer("horizontalModeMasteries", 0.5, function(elapsed) if horizontalModeMasteries then horizontalModeMasteries(elapsed) end end)
MawAddTimer("MawRegen",               0.1, function(elapsed) if MawRegen               then MawRegen(elapsed)               end end)
MawAddTimer("leecher",                0.5, function(elapsed) if leecher                then leecher(elapsed)                end end)
MawAddTimer("checkOutOfBound",        2.0, function(elapsed) if checkOutOfBound        then checkOutOfBound(elapsed)        end end)
MawAddTimer("eliteRegen",             0.1, function(elapsed) if eliteRegen             then eliteRegen(elapsed)             end end)
MawAddTimer("mappingRegen",           1.0, function(elapsed) if mappingRegen           then mappingRegen(elapsed)           end end)
MawAddTimer("checkMapCompletition",  10.0, function(elapsed) if checkMapCompletition  then checkMapCompletition(elapsed)  end end)
MawAddTimer("nightmare",              0.5, function(elapsed) if nightmare              then nightmare(elapsed)              end end)
MawAddTimer("elementalBuffs",         1.0, function(elapsed) if elementalBuffs         then elementalBuffs(elapsed)         end end)
MawAddTimer("mawBuffApply",           0.5, function(elapsed) if mawBuffApply           then mawBuffApply(elapsed)           end end)
MawAddTimer("elementalistStacksDecay",0.1, function(elapsed) if elementalistStacksDecay then elementalistStacksDecay(elapsed) end end)
MawAddTimer("poisonTimer",            1.0, function(elapsed) if poisonTimer            then poisonTimer(elapsed)            end end)
