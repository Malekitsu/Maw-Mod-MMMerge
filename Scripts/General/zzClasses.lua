...
function events.GameInitialized2()
	...
	function events.CalcDamageToMonster(t)	
		...
		if data and data.Player and table.find(shamanClass, data.Player.Class) and t.DamageKind==4 and data.Object==nil then	
			...
			local fireDamage=(m1^2/pl.LevelBase^2/2)
...
local function shamanSkills(isShaman, id)
	if isShaman then
		...
		local txt
		local fireDamage=math.round(m1^2/pl.LevelBase^2/2*10000)/100
		...
		local armsmasterDamage=math.round(m4^2.65/(10+pl.LevelBase)*10)
