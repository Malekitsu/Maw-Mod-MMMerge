...
do
	-- Rod:
	-- In rare cases, when Game.CurrentPlayer does not match player who's inventory is open, UseMouseItem event gives error
	-- Is not supposed to ever happen in solo game, but happens in multiplayer
	local InvPlayerId
	function events.LoadInventoryPics(Player, PlayerIndex) -- merge specific event (RemoveItemLimits.lua)
		InvPlayerId = PlayerIndex
	end
	-- Rod.
	...
	mem.hookfunction(mmv(0x459040, 0x4680F1, 0x466572), 1, 2, function(d, def, this, player, portrait)
		...
		if player == 0 then
			player = 1
			portrait = Game.CurrentCharPortrait -- merge specific field (CharacterOutfits.lua)
			for i, v in Party.PlayersIndexes do
				if v == InvPlayerId then
					player = i + 1
					break
				end
			end
...
do
	-- Rod:
	--[[
	Beginning of 0x453ECC is:
	push ebp
	mov ebp, esp
	sub esp, 0xc94

	0xc94 is amount of items in items.txt multiplied by 4
	I have to modify this constant in RemoveItemLimits.lua.
	Hookfunction makes this constant inacessibe for me, and
	this all happens here, before i can inject any code to bypass it. Dead end.
	Wrapping this hook in GameInitialized2 event.

	Also, using mem.hookfunction instead of hookmanager, because event handlers, that are set
	before events.GameInitialized2, won't activate hook.
	]]
	function events.GameInitialized2()
		--local hooks = HookManager()
		--hooks.hookfunction(mmv(0x448790, 0x45664C, 0x453ECC), 1, mmv(3, 3, 4), function(d, def, this, strength, kind, item, alwaysEnchant)
		mem.hookfunction(mmv(0x448790, 0x45664C, 0x453ECC), 1, mmv(3, 3, 4), function(d, def, this, strength, kind, item, alwaysEnchant)
			local t = {
				-- :structs.Item
...
mem.hookfunction(mmv(0x403050, 0x402D6E, 0x402E78), 1, 0, function(d, def, index)
	...
	if mmver == 8 then
		...
		events.cocalls("MonsterKilled", Map.Monsters[index], index, callDef, killer)
		-- ~cthscr
	else
...
do
	...
	local function Who(i, action)
		...
		-- Rod: early exit and logging
		if i < 0 then
			_G.Log(_G.Merge.Log.Error, "Negative attacker ref in events.lua Who(i, action): %s", i)
			return t
		end
		-- Rod.
		...
		elseif kind == 3 then
			-- Rod:
			-- Adding logging and check untill i figure out why this happening
			if i >= Map.Monsters.count then
				_G.Log(_G.Merge.Log.Error, "Error in attacker recognition handler: invalid monster id: %s / %s", i, Map.Monsters.count)
				i = 0
			end
			-- Rod.
	...
	local function HitMon(d, def, attackerID, monIndex, speed, action)
		...
		-- Rod: encountered console popup once
		if monIndex >= Map.Monsters.count then
			_G.Log(_G.Merge.Log.Error, "Error in monster under attack recognition handler: invalid monster id: %s / %s", monIndex, Map.Monsters.count)
			return
		end
		-- Rod.
	...
	local function HitPlayer(d, def, attackerID, action, speed, slot)
		...
		local t = {
			...
			-- Rod: assign these fields in check below
			---- :structs.Player
			--Player = Party[slot],
			--PlayerSlot = slot,
			-- Rod.
		...
		-- Rod: check slot, and log, assign default target if necessary
		if slot < Party.count and slot >= 0 then
			-- :structs.Player
			t.Player = Party[slot]
			t.PlayerSlot = slot
		else
			_G.Log(_G.Merge.Log.Error, "Error in HitPlayer handler: invalid player slot: %s / [0, %s]", slot, Party.count)
			t.Player = Party[0]
			t.PlayerSlot = 0
		end
		-- Rod.
